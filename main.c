...
Gene_Type initGene()
{
    ...
    // set unevaluated fitnes value to 0
    NewGene.fitness = 0.0;
...
void EvaluateGene( Gene_Type* Gene ,Population_Type* Pop){
    ...
    if(Gene)
    {
		...
		Pop->CurrentEvals++;
        	...
        	Gene->fitness = 0.0;
 	        coeff_SetCurrentDiffusion ((double)Gene->params[0]);
                coeff_SetCurrentSpread ((double)Gene->params[1]);
                coeff_SetCurrentBreed ((double)Gene->params[2]);
                coeff_SetCurrentSlopeResist ((double)Gene->params[3]);
                coeff_SetCurrentRoadGravity ((double)Gene->params[4]);   
								...
								float         MutRate)
{
    ...
    if( NewPopulation != NULL )    
    {
        ...
        NewPopulation->NumSubPopulations = 6;
...
void GenerateOffspring( Population_Type* Pop , unsigned int NumOffSpring ){
	...
	printf("Total fitness=%f", TotalFitness);
...
/* Initialize stored gene */
	...
	sprintf(filename,"%slog_%d_%d_%d_%1.2f",scen_GetOutputDir(),PopulationSize,Generations,NumReplaced,MutationRate);
	pFile = fopen(filename,"w");
	sprintf(filename1,"%sstats_%d_%d_%d_%1.2f",scen_GetOutputDir(),PopulationSize,Generations,NumReplaced,MutationRate);
  ...
  main (int argc, char *argv[])
{
  ...
  if (( strcmp (argv[1], "evolve") == 0 ) & (argc != 9))
  {
    print_usage (argv[0]);
  }
  ...
  scen_init (argv[2]);
/* if there are 9 arguments, use the parameters from the run commnds, otherwise use defaults */
if (argc == 9) {
